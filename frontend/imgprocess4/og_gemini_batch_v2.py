...
def analyze_image(path):
    ...
    if obj.get("category") not in ["Hoodie","Tee","Cap","Slide"]:
        obj["category"] = "Tee"
    if obj.get("view") not in ["front","back","detail"]:
        obj["view"] = "front"
    ...
    concept = obj.get("concept_name") or "Unnamed"
    ...
    # enforce .11 endings
    def to_11(x): 
        # round to nearest integer then add .11 semantics
        base = round(float(x))
        return float(f"{base}.11")
    if price <= 0:
        # fallback safety by category (won’t usually trigger)
    ...
    if badge == "Vault Exclusive":
        # keep higher within category safe range
...
def ensure_dirs():
    ...
    for sub in ["front","back","detail"]:
        Path(f"{OUT_DIR}/{sub}").mkdir(parents=True, exist_ok=True)

def organize_file(src_path, view, handle_fragment):
    # copies image into out/front|back|detail/<handle_fragment>/
    dest_root = f"{OUT_DIR}/{view}/{handle_fragment}"
    Path(dest_root).mkdir(parents=True, exist_ok=True)
    shutil.copy2(src_path, f"{dest_root}/{os.path.basename(src_path)}")

def admin_shape_from_group(group):
    """
    Collapse multiple views (front/back/detail) into a single product payload
    for Admin API upsert: one variant per concept (you can extend later).
    """
    any_obj = group[0]
...
def main():
    ...
    if not files:
        print("No images found in ./images"); return
    ...
    for f in files:
        try:
            ...
            organize_file(f, obj["view"], obj["handle_fragment"])
            # group for admin shape
            key = (obj["category"], obj["handle_fragment"])
            groups.setdefault(key, []).append(obj)
            print("✓", os.path.basename(f), "→", obj["category"], obj["view"], obj["price_inr"])
            time.sleep(0.2)
    ...
    print("Organized images under:", OUT_DIR, "/{front,back,detail}/<handle_fragment>/")
