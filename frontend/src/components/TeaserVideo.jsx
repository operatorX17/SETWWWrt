...
const TeaserVideo = ({ onEnterVault }) => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black">
      ...
      <div 
        ...
        style={{
          backgroundImage: 'linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.4)), url("https://images.unsplash.com/photo-1518709268805-4e9042af2176?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80")',
          filter: videoLoaded ? 'opacity(0.2)' : 'opacity(1)',
          transition: 'opacity 2s ease-in-out'
        }}
      />
      
      {/* YouTube Video Embed - Working version */}
      {!videoError && (
        <iframe
          className={`absolute inset-0 w-full h-full transition-opacity duration-2000 ${
            videoLoaded && !videoError ? 'opacity-90' : 'opacity-0'
          }`}
          src="https://www.youtube.com/embed/ePOglweqy7o?autoplay=1&mute=1&loop=1&playlist=ePOglweqy7o&controls=0&showinfo=0&rel=0&iv_load_policy=3&modestbranding=1&start=16&enablejsapi=1"
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-black/30 via-transparent to-black/50 flex items-center justify-center">
        <div className="text-center text-white z-10 max-w-4xl px-6">
          ...
          {/* Modern CTA - Now triggers navigation */}
          <button 
